...
const SCROLL_HIDE_THRESHOLD = 80;  // scroll down past this → hide navbar (only category bar sticks)
const SCROLL_SHOW_THRESHOLD = 50;  // near top → always show both; scroll up → show both
const SCROLL_DELTA_MIN = 6;        // min px to treat as scroll direction (avoids jitter)
const SCROLL_TICK_MS = 120;        // throttle scroll handler
...
const Header = () => {
  ...
  // Scroll down → only category bar sticks; scroll up → show full header. Throttled + hysteresis to prevent vibration.
  useEffect(() => {
    const update = () => {
      ...
      if (delta >= SCROLL_DELTA_MIN && y > SCROLL_HIDE_THRESHOLD) {
        setShowNavbar(false);
        return;
      }
    ...
    const requestTick = () => {
      ...
      const now = Date.now();
      if (now - lastTick.current >= SCROLL_TICK_MS) {
        ticking.current = true;
        requestAnimationFrame(update);
      } else {
        ticking.current = true;
        const delay = Math.max(0, SCROLL_TICK_MS - (now - lastTick.current));
        setTimeout(() => {
          lastTick.current = Date.now();
          update();
          ticking.current = false;
  ...
  return (
    <header className="w-full z-50 sticky top-0" style={{ backgroundColor: 'var(--bg-primary)' }}>
      {/* Navbar + mobile search: collapse when scrolling down, show when scrolling up or at top */}
      <div
        className="overflow-hidden transition-all duration-300 ease-in-out"
        style={{ maxHeight: showNavbar ? '400px' : '0' }}
      >
        <Navbar 
      ...
      {/* Category bar – always visible when sticky; sticks when navbar is collapsed */}
      <CategoryBar />
